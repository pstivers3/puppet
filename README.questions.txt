...
lunchprint { $lunch: }
  ...
  …
		...
		Clearly $::<var name> refernces an internal variable. Not sure exactly what the :: does. Name space? Removing :: didn't have any effect.
		
	chap 3, definitions
	
		Is ${name} a special variable?
		looks like the array is passed through the tmpfile "function" to ${name}, though name isn't explicitly defined as an input variable. No tmpfile(name)
  ...
  file { "/tmp/${name}":
    content => "Hello, world\n",
		...
		Changing the variable name from 'name' to 'ame' causes an error.
			...
			Error: Duplicate declaration: File[/tmp/] is already declared in file /home/ubuntu/puppet/manifests/test.pp:4; cannot redeclare at /home/ubuntu/puppet/manifests/test.pp:4 on node pc1.ec2.internal
			Error: Duplicate declaration: File[/tmp/] is already declared in file /home/ubuntu/puppet/manifests/test.pp:4; cannot redeclare at /home/ubuntu/puppet/manifests/test.pp:4 on node pc1.ec2.internal
		
	chap 3, dependencies
	
		How come puppet doesn't have all dependencies buit in under the hood? Could puppet at least infer the dependencies based in a directory structure including the same name as the package?
		...
		    # File['/etc/ntp.conf'] ~> Package['ntp'] -> Service['ntp'], # swap position of File and Package as a test. Same syntax error.
		  ...
		  returns:
			...
			Error: Syntax error at ','; expected '}' at /home/ubuntu/puppet/modules/admin/manifests/ntp.pp:21 on node pc2.ec2.internal
	...
	chap 3 run stages
		...
		Why doesn't this run?
			...
			Error: Could not parse for environment production: Syntax error at '.'; expected '}' at /home/ubuntu/puppet/manifests/run_stages.pp:3 on node pc1.ec2.internal
		
		This doesn't return anything. Why not?
		
			ubuntu@pc1:~/puppet/manifests$ puppet apply stages.pp 
			Notice: Compiled catalog for pc1.ec2.internal in environment production in 0.01 seconds
	...
	chapter 3, parameterization
		...
		What does this error mean? Why won't this run?
			...
			Error: Could not parse for environment production: Classes, definitions, and nodes may only appear at toplevel or inside other classes at /home/ubuntu/puppet/manifests/param.pp:3 on node pc1.ec2.internal
